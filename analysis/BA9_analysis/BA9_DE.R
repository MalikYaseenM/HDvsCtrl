...
count_matrix <- '../../samples/Analysis_Results/BA9_raw_filter.csv'
...
means <- aggregate(colData[, 3], list(colData$Subject.subject_type), mean, na.rm =TRUE, na.action = na.pass)
colData <- within(colData, Subject.death[Dataset.dataset_id == 'C_0054_BA9_mRNASeq'] <- round(means[1,2], 0))
